    ...
    for i, (step_data, side) in enumerate(ordered_steps):
        try:
            ...
            if side == 'Left':
                ...
                if len(peaks) < 2 or (step_data['RightPlateGRF'][peaks[0]:peaks[1]].mean()/weight_N >= 0.02):
                    bad_step_indices.add(i)
            else:
                peaks, _ = find_peaks(step_data['RightPlateGRF'], distance=150/(1000/Sampling_Rate), height=0.7*weight_N)
                if len(peaks) < 2 or (step_data['LeftPlateGRF'][peaks[0]:peaks[1]].mean()/weight_N >= 0.02):
                    bad_step_indices.add(i)
